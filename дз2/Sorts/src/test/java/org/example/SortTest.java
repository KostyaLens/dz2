package org.example;

import org.junit.*;

public class SortTest {

    @Test
    public void bubbleSortTest1() { //Случайный Массив
        int[] a = new int[]{4, 6, 1, 2, 6, 0, 432};
        int[] res = new int[]{0, 1, 2, 4, 6, 6, 432};
        BubbleSort Bsort = new BubbleSort();
        Assert.assertArrayEquals(res, Bsort.BubbleSort(a));
    }
    @Test
    public void bubbleSortTest2() {//Отсортированный  Массив
        int[] a = new int[]{0, 1, 2, 4, 6, 6, 432};
        int[] res = new int[]{0, 1, 2, 4, 6, 6, 432};
        BubbleSort Bsort = new BubbleSort();
        Assert.assertArrayEquals(res, Bsort.BubbleSort(a));
    }
    @Test
    public void bubbleSortTest3() {//Обратыный Массив
        int[] a = new int[]{432, 6, 6, 4, 2, 1, 0};
        int[] res = new int[]{0, 1, 2, 4, 6, 6, 432};
        BubbleSort Bsort = new BubbleSort();
        Assert.assertArrayEquals(res, Bsort.BubbleSort(a));
    }
//    @Test
//    public void bubbleSortTest4() {//Тест который не пройдёт
//        int[] a = new int[]{4, 6, 1, 2, 6, 0, 432};
//        int[] res = new int[]{0, 1, 4, 2, 6, 6, 432};
//        BubbleSort Bsort = new BubbleSort();
//        Assert.assertArrayEquals(res, Bsort.BubbleSort(a));
//    }

}